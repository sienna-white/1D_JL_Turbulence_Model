...
function initialize_turbulent_functions(discretization, N_BV2, kappa=kappa)
    Q2  = random_seed_vector(discretization)   # "seed" the turbulent field with small values, then let it evolve
    Q2L = random_seed_vector(discretization)
    Q = sqrt.(Q2)
    z = discretization["z"]
    H = discretization["H"]
...
function random_seed_vector(discretization, SMALL=SMALL)
   return zeros(discretization["N"]) .+ SMALL
end 
...
function calculate_turbulent_functions(gh, Q, L, discretization)
    ...
    for i in 1:N 
        ...
        Kq[i] = sh*Q[i]*L[i] + nu    # Turbulent diffusivity for Q
        Kz[i] = sh*Q[i]*L[i] + nu 
    end

    # Clip the bottom of Kz to be > SMALL
    Kz = clamp.(Kz, SMALL, Inf)
